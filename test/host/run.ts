/** Build a fixture workspace and launch the VS Code extension-host tests. */
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        ...
        'cppTestExplorer.debug': {
          lldb: { terminal: 'console' },
          cppdbg: { externalConsole: false },
        },
      }),
    );
    const extensions = path.join(root, 'extensions');
    await fs.mkdir(extensions);
    const debugExtensions = (process.env.CPP_TEST_DEBUG_EXTENSIONS ?? '')
      .split(path.delimiter)
      .filter(Boolean);
    for (const extension of debugExtensions)
      await fs.symlink(extension, path.join(extensions, path.basename(extension)), 'dir');
    delete process.env.ELECTRON_RUN_AS_NODE;
    await runTests({
      vscodeExecutablePath: process.env.CPP_TEST_VSCODE_PATH,
      extensionDevelopmentPath: path.resolve('.'),
      extensionTestsPath: path.resolve('out/test/host/suite'),
      launchArgs: [
        workspace,
        '--disable-gpu',
        '--user-data-dir',
      ...
      extensionTestsEnv: {
        CPP_TEST_WITH_DEBUGGERS: debugExtensions.length ? '1' : '',
        CPP_TEST_SOURCE: path.resolve('test/fixtures/cmake/tests.cpp'),
      },
    });
  } finally {
