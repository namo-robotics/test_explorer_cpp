...
export interface TreeNode<T> {
  id: string;
  parent?: T;
  children: { forEach(callback: (item: T) => void): void };
}
/** Expand selected nodes into unique cases while pruning excluded ancestors. */
export function selectLeaves<T extends TreeNode<T>>(
  roots: T[],
  include: readonly T[] | undefined,
  ...
  const visit = (item: T) => {
    for (let ancestor: T | undefined = item; ancestor; ancestor = ancestor.parent) {
      if (excluded.has(ancestor.id)) {
        return;
      }
    }
    if (isLeaf(item)) {
