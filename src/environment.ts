...
export async function workspaceEnvironment(
  ...
  if (settings.autoSourceWorkspace && existsSync(automatic) && !scripts.includes(automatic)) {
    scripts.push(automatic);
  }
  if (!scripts.length) {
  ...
  const result = await runProcess(
    ...
    [
      ...
      '--norc',
      '-c',
      'for script in "$@"; do source "$script" 1>&2 || exit $?; done; /usr/bin/env -0',
      'cpp-test-explorer',
      ...scripts,
    ],
  ...
  if (result.code !== 0 || result.cancelled || result.timedOut || result.truncated) {
    throw new Error(
      `Could not source ROS setup scripts: ${result.stderr || (result.timedOut ? 'timed out' : 'cancelled or output too large')}`,
    );
  }
  return Object.fromEntries(
    result.stdout
...
export function applyEnvironment(
  ...
  for (const entry of assignments) {
    ...
    if (i < 1) {
      throw new Error(`Invalid environment assignment: ${entry}`);
    }
  ...
  for (const entry of modifications) {
    ...
    if (!match) {
      throw new Error(`Invalid CTest environment modification: ${entry}`);
    }
    ...
    switch (op) {
      ...
      case 'cmake_list_append':
        env[key] = env[key] ? env[key] + (op.startsWith('path') ? ':' : ';') + value : value;
        break;
      case 'path_list_prepend':
      case 'cmake_list_prepend':
        env[key] = env[key] ? value + (op.startsWith('path') ? ':' : ';') + env[key] : value;
        break;
      default:
        throw new Error(`Unsupported CTest environment operation: ${op}`);
    }
  }
  ...
  for (const entry of append) {
    const i = entry.indexOf('=');
    if (i < 1) {
      throw new Error(`Invalid ament environment assignment: ${entry}`);
    }
