...
export function settingsFor(folder: vscode.WorkspaceFolder): Settings {
  ...
  const settings: Settings = {
    sourceRoots: config.get('sourceRoots', ['src']), buildBase: config.get('buildBase', 'build'),
  ...
  if (!Number.isInteger(settings.concurrency) || settings.concurrency < 0) throw new Error('concurrency must be a nonnegative integer');
  settings.concurrency ||= Math.max(1, Math.min(4, os.availableParallelism()));
  if (!(settings.discoveryTimeout > 0) || !Number.isFinite(settings.discoveryTimeout)) throw new Error('discoveryTimeout must be positive');
  if (settings.timeout !== null && (!Number.isFinite(settings.timeout) || settings.timeout < 0)) throw new Error('timeout must be null or a nonnegative number');
  if (!['case', 'executable'].includes(settings.parallelMode)) throw new Error('parallelMode must be executable or case');
  for (const key of ['sourceRoots', 'buildDirectories', 'exclude', 'setupScripts'] as const) {
    if (!Array.isArray(settings[key]) || settings[key].some(value => typeof value !== 'string')) throw new Error(`${key} must contain strings`);
  }
  if (!Array.isArray(settings.executables)) throw new Error('executables must be an array');
  for (const entry of settings.executables) {
    if (!entry || typeof entry.id !== 'string' || typeof entry.path !== 'string' ||
      ...
      (entry.timeout !== undefined && (!Number.isFinite(entry.timeout) || entry.timeout < 0))) throw new Error('Invalid explicit executable configuration');
  }
  return settings;
}
